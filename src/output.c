...
print_pure_menu (FILE * fp, char *now)
{
  ...
  fprintf (fp, "</a>");
  fprintf (fp, "<ul>");
  fprintf (fp, "<li><a href=\"#\">总览</a></li>");
  fprintf (fp, "<li><a href=\"#%s\">独立访客信息</a></li>", VISIT_ID);
  fprintf (fp, "<li><a href=\"#%s\">URL访问统计</a></li>", REQUE_ID);
  fprintf (fp, "<li><a href=\"#%s\">静态资源访问</a></li>", STATI_ID);
  fprintf (fp, "<li><a href=\"#%s\">404页面访问</a></li>", FOUND_ID);
  fprintf (fp, "<li><a href=\"#%s\">访客IP统计</a></li>", HOSTS_ID);
  fprintf (fp, "<li><a href=\"#%s\">访客操作系统</a></li>", OPERA_ID);
  fprintf (fp, "<li><a href=\"#%s\">访客浏览器</a></li>", BROWS_ID);
  fprintf (fp, "<li><a href=\"#%s\">时间分布请看</a></li>", VTIME_ID);
  fprintf (fp, "<li><a href=\"#%s\">来源页面URLs</a></li>", REFER_ID);
  fprintf (fp, "<li><a href=\"#%s\">来源网站域名</a></li>", SITES_ID);
  if (!ignore_panel (KEYPHRASES))
    fprintf (fp, "<li><a href=\"#%s\">Google搜索关键字</a></li>", KEYPH_ID);
#ifdef HAVE_LIBGEOIP
  fprintf (fp, "<li><a href=\"#%s\">访客地理位置</a></li>", GEOLO_ID);
#endif
  fprintf (fp, "<li><a href=\"#%s\">HTTP状态码</a></li>", CODES_ID);
  fprintf (fp, "<li class=\"menu-item-divided\"></li>");

  fprintf (fp, "</ul>");
  fprintf (fp, "<p>Generated by<br /><br />Goaccess.hackinglab.cn<br />—————————————————<br />中文版(%s) %s</p>",
           GO_VERSION, now);
  fprintf (fp, "</div>");
  fprintf (fp, "</div> <!-- menu -->");
...
print_html_common (FILE * fp, GHolder * h, int processed, const GOutput * panel)
{
  ...
  fprintf (fp, "<th>访客数</th>");
  fprintf (fp, "<th>点击数</th>");
  fprintf (fp, "<th>百分比%%</th>");
  fprintf (fp, "<th>流量消耗</th>");

  if (conf.serve_usecs)
    fprintf (fp, "<th>服务耗时</th>");
  if (conf.append_protocol && panel->protocol)
    fprintf (fp, "<th>协议版本</th>");
  if (conf.append_method && panel->method)
    fprintf (fp, "<th>请求方法</th>");
